...
import { Mail, Phone, MapPin, Link as LinkedIn, Video, Play, ExternalLink, Award, GraduationCap, Briefcase, Code, User, Languages, Globe } from 'lucide-react';
      ...
      case 'summary':
        return summary && (
          <section key="summary" className={isProfessional ? "mb-4" : "mb-5"}>
            ...
            <p className="text-[10px] text-justify leading-relaxed text-slate-800">{summary}</p>
          </section>
        );
          ...
          <section key="education" className={isProfessional ? "mb-4" : "mb-5"}>
            ...
            {education.map((edu, index) => (
              <div key={index} className="mb-3">
                ...
                <div className="text-[10px] text-gray-700">{edu.institution}, {edu.location}</div>
                {edu.gpa && <p className="text-[10px] text-gray-700 mt-0.5">GPA: {edu.gpa}</p>}
                {edu.details && <p className="text-[10px] text-gray-700 mt-0.5 leading-tight">{edu.details}</p>}
              </div>
            ))}
          </section>
        );
              ...
              <div key={index} className="mb-3">
                ...
                <ul className="list-disc list-outside ml-4 mt-1 space-y-1">
                  {intern.points?.map((point, pIndex) => (
                    <li key={pIndex} className="text-[10px] text-gray-700 leading-relaxed pl-1">{point}</li>
                  ))}
                </ul>
              </div>
            ))}
          ...
          <section key="experience" className={isProfessional ? "mb-4" : "mb-5"}>
            ...
            {experience.map((exp, index) => (
              <div key={index} className="mb-4">
                ...
                <div className="text-[10px] text-gray-700 mb-2">{exp.company} | {exp.location}</div>
                <div className="space-y-3">
                  {exp.projects?.map((proj, pIndex) => (
                    <div key={pIndex} className={cn(
                      ...
                      <p className="text-[10px] text-gray-700 mt-1 leading-relaxed ml-3">{proj.description}</p>
                    </div>
                  ))}
                </div>
              </div>
          ...
          <section key="projects" className={isProfessional ? "mb-4" : "mb-5"}>
            ...
            {projects.map((proj, index) => (
              <div key={index} className="mb-3">
                ...
                <p className="text-[10px] text-gray-700 mt-1 leading-relaxed">{proj.description}</p>
                {proj.links && (
          ...
          <section key="certifications" className={isProfessional ? "mb-4" : "mb-5"}>
            ...
            <div className="grid grid-cols-1 gap-1">
              {certifications.map((cert, index) => (
                <div key={index} className="text-[10px] text-gray-700 flex justify-between">
    ...
    if (template === 'professional') {
      return (
        <header className="mb-6 text-center space-y-1">
          ...
          <div className="flex flex-wrap justify-center gap-x-3 gap-y-1 text-[10.5px] mt-2 text-gray-600">
          ...
          <div className="flex flex-wrap justify-center gap-x-3 gap-y-1 text-[10.5px] mt-1 text-blue-700">
    ...
    return (
      <header className={cn(
        ...
        <div className={cn(
          ...
          {personalInfo.linkedin && (
            <a href={personalInfo.linkedin} target="_blank" rel="noreferrer" className="flex items-center gap-1.5 hover:underline">
